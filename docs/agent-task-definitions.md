...
description: "How to define OpCon MFT Agent tasks, including source, destination, compression, encryption, and file renaming configuration."
tags:
  - Reference
  - Procedural
  - Automation Engineer
...
- Use this when understanding how task steps are ordered and how the file set progresses through each step
...
A file set is the group of files that a task step is currently working on. Steps such as `get` add files to the file set, while steps such as `compress` and `encrypt` replace input files with output files in the set. The file set is stored locally in the `Data\` directory for the corresponding job to support restart capability.
...
The **Reverse Order** option changes the step sequence for compression and encryption. By default, a send task compresses first and then encrypts. If the receiving system requires the file to be encrypted before compression, select **Reverse Order** to change the sequence.

**What does the Timeout field control in the Source section?**

The **Timeout** field sets how many minutes the `get` step waits for source files if no matching files are present when the task starts. If no files are found within this period, the step returns an error and the remaining steps are not processed. The default value is 1 minute.

## Glossary

**File Set** — The collection of files that are actively being processed by the current task step. Each step receives the file set from the previous step and may add, replace, or remove files from it.

**get step** — The step that retrieves files from the source endpoint and creates the initial file set for the task.

**put step** — The step that delivers the current file set to the destination endpoint.

**Related topics:**

- [Endpoint Definitions](./agent-endpoint-definitions.md)
- [Encryption Definitions](./agent-encryption-definitions.md)
- [Group Definitions](./agent-group-definitions.md)
